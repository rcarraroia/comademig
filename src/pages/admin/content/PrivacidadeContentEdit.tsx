...
import { usePrivacidadeContent } from "@/hooks/useLegalPages";
...
interface PrivacidadeForm {
  title: string;
  sections: SectionForm[];
}

const PrivacidadeContentEdit = () => {
  const { isAdmin, loading: authLoading } = useAuth();
  const { data: pageData, isLoading } = usePrivacidadeContent();
  const queryClient = useQueryClient();
  const [isSaving, setIsSaving] = useState(false);

  const { register, control, handleSubmit, reset } = useForm<PrivacidadeForm>({
    defaultValues: {
      title: pageData?.content_json.title || "Política de Privacidade",
  ...
  const onSubmit = async (data: PrivacidadeForm) => {
    setIsSaving(true);
    try {
      const contentJson = {
        title: data.title,
      ...
      queryClient.invalidateQueries({ queryKey: ['content', 'privacidade'] });
    ...
    <div className="container mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-gray-600 font-inter">
          Edite o conteúdo da Política de Privacidade (LGPD)
        </p>
      </div>
    ...
    <div className="border rounded-lg p-4 space-y-4 bg-gray-50">
      ...
      <div>
        ...
        <Input
          ...
          placeholder="Ex: 1. Informações que Coletamos"
        />
      </div>
...
export default PrivacidadeContentEdit;
