...
import { CheckCircle, Clock, AlertCircle, Download, Copy, QrCode, FileText } from 'lucide-react';
...
interface CertidaoPaymentResultProps {
  paymentData: {
    payment: any;
    customer: any;
...
export function CertidaoPaymentResult({ 
  paymentData, 
  onPaymentConfirmed 
}: CertidaoPaymentResultProps) {
  const { payment, serviceData, paymentMethod, originalValue, finalValue, pixDiscount } = paymentData;
    ...
    switch (status) {
      ...
      case 'RECEIVED':
        return {
          ...
          description: 'Sua solicitação de certidão foi registrada!'
        };
      case 'OVERDUE':
        return {
  ...
  const getCertidaoDisplayName = (tipo: string): string => {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            <div>
              <p className="text-sm font-medium text-muted-foreground">Tipo de Certidão</p>
              <p className="font-semibold">{getCertidaoDisplayName(serviceData.tipo_certidao)}</p>
            </div>
            
            <div>
              <p className="text-sm font-medium text-muted-foreground">Justificativa</p>
              <p className="text-sm bg-muted p-3 rounded">{serviceData.justificativa}</p>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Valor Original</p>
                <p className="font-semibold">{formatCurrency(originalValue)}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-muted-foreground">Valor Final</p>
                <p className="font-semibold text-green-600">{formatCurrency(finalValue)}</p>
              </div>
            </div>
      ...
      {paymentMethod === 'pix' && payment.pixTransaction && (
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {pixDiscount && pixDiscount > 0 && (
              <div className="bg-green-50 p-4 rounded-lg">
                <p className="text-sm text-green-800">
                  <strong>Desconto de 5% aplicado!</strong> Você economizou {formatCurrency(pixDiscount)} pagando com PIX.
                </p>
              </div>
            )}
          </CardContent>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {payment.installmentCount && payment.installmentCount > 1 && (
                <div>
                  ...
                  <p className="font-semibold">{payment.installmentCount}x de {formatCurrency(finalValue / payment.installmentCount)}</p>
                </div>
              )}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              <p>5. A certidão será processada pela equipe administrativa</p>
            </div>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-green-200">
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              ...
              <p>✅ Você receberá atualizações sobre o andamento</p>
              <p>✅ A certidão será processada em até 5 dias úteis</p>
            </div>
            
            <div className="mt-4 pt-4 border-t">
              <Button 
                onClick={() => window.location.href = '/dashboard/certidoes'}
                className="w-full"
              >
