...
serve(async (req) => {
    ...
    try {
        ...
        // Validar método de pagamento baseado no ciclo
        if (billingType === 'BOLETO') {
            throw new Error('Boleto não é mais aceito. Use PIX (apenas planos anuais) ou Cartão de Crédito.')
        }

        // PIX apenas para planos anuais
        if (billingType === 'UNDEFINED' && cycle !== 'YEARLY') {
            throw new Error('PIX disponível apenas para planos anuais. Para planos mensais e semestrais, use Cartão de Crédito.')
        }

        // Cartão obrigatório para planos mensais e semestrais
        if ((cycle === 'MONTHLY' || cycle === 'SEMIANNUALLY') && billingType !== 'CREDIT_CARD') {
            throw new Error('Planos mensais e semestrais requerem Cartão de Crédito para pagamento automático.')
        }
